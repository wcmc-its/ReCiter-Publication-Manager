...
export const updateAffiliationType = (affiliationTypes) => dispatch => {
    console.log("affili action", affiliationTypes)
    dispatch({
        type: methods.UPDATE_AFFILIATION_TYPES,
        payload: affiliationTypes
    })
}

export const clearDeptPersonAffiliTypesData = () => dispatch => {
    dispatch({
        type: methods.CLEAR_DEPT_TYPES_DATA
    })
    dispatch({
        type: methods.CLEAR_PERSON_TYPES_DATA
    })
    dispatch({
        type: methods.CLEAR_AFFILIATION_TYPES_DATA
    })
}
...
export const getGroupReviewSuggestions = (deptTypes, personTypes, affiliationTypes, cb) => dispatch => {
    ...
    // })
    // cb()
    let url = '/api/reciter/feature-generator/by/group?'
    if (personTypes.length) url += 'personType=' + String(personTypes)
    if (deptTypes.length || affiliationTypes.length) url += '&'
    if (deptTypes.length) url += 'departmentalAffiliation=' + String(deptTypes)
    if (affiliationTypes.length) url += '&'
    if (affiliationTypes.length) url += 'organizationalAffiliation=' + String(affiliationTypes)
    fetchWithTimeout(url, {
        credentials: "same-origin",
        method: 'GET',
        headers: {
