 ...
 *     status was established by hand on 2026-08-29. Needs AWS credentials (pod IRSA in
...
const PRESENT = ['brf9046', 'ack7001', 'stetson']
...
try {
  ...
  const resp = await ddb.send(new BatchGetItemCommand({
    RequestItems: { Identity: { Keys: [...PRESENT, ...ABSENT].map((uid) => ({ uid: { S: uid } })), ProjectionExpression: 'uid' } },
  }))
